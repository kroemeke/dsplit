...
def newvolume(output, volume, volsize, thisvolfiles, thisvoldirs):
    ...
    if volume >= 0:
        ...
        stdout.write(
            (BD + "Volume %02d:" + RE + "%s") % (volume, verbose and "\n" or "")
        )
...
try:
    (popts, argv) = getopt(
        argv[1:], "s:vVh", ["volume-size=", "for-cd", "for-dvd", "for-bdr"]
    )
...
for root, dirs, files in os.walk(input, True):
    ...
    for f in files:
        ...
        if s > maxSize:
            ...
            (voldir, volume, volsize) = newvolume(
                output, volume, volsize, thisvolfiles, thisvoldirs
            )
            targetdir = path.join(voldir, root.replace(input, "", 1))
            thisvolfiles = 0
            thisvoldirs = 0
